...
class SliderRevolution(models.Model):
    SLIDER_TYPE_CHOICES = (
        ('standard', _("Standard Slider Type")),
        ('hero', _('Hero Block Type')),
        ('carousel', _('Carousel Type')),
    )

    SLIDER_LAYOUTS_CHOICES = (
        ('auto', _("Auto Responsive Layout")),
        ('fullwidth', _('Full Width Layout')),
        ('fullscreen', _('Full Screen Layout')),
    )

    SLIDER_AUTO_HEIGHT_CHOICES = (
        ('on', _("Auto Height ON")),
        ('off', _('Auto Height OFF')),
    )

    title = models.CharField(
        help_text=_("Slider Revolution"),
        max_length=255,
        default="Slider Revolution section X",
    )

    type = models.CharField(max_length=255, choices=SLIDER_TYPE_CHOICES, default=SLIDER_TYPE_CHOICES[0])
    layout = models.CharField(max_length=255, choices=SLIDER_LAYOUTS_CHOICES, default=SLIDER_LAYOUTS_CHOICES[0])
    autoHeight = models.CharField(max_length=255, choices=SLIDER_AUTO_HEIGHT_CHOICES, default=SLIDER_AUTO_HEIGHT_CHOICES[0])
    delay = models.IntegerField(default=9000, blank=True, null=True)
    gridwidth = models.IntegerField(default=1040, blank=True, null=True)
    gridheight = models.IntegerField(default=720, blank=True, null=True)

    def render(self):
        template = Template(""""""<div id="revolutionSlider" class="slider rev_slider"
                               data-plugin-revolution-slider data-plugin-options=
                               '{"delay": {{ delay }}, "sliderType": {{ type }}, "sliderLayout": {{ layout }},
                               "gridwidth": {{ gridwidth }}, "gridheight": {{ gridheight }}}'>)

                               {{ % FOR slide in self.slide % }}
                               <div ... {{ slide.option }}

                                    {{ % FOR layer in slide.layer % }}
                                    <div ... {{ layer.option }}
        ...
        c = Context({"delay": self.delay, "sliderType": self.type, })
...
class Slide(models.Model):
    ...
    image = models.ImageField(upload_to="static/media/revolution-slider/%Y/%m/%d", blank=True, null=True)
    transition = models.CharField(max_length=255, choices=SLIDE_TRANSITION_CHOICES, default=SLIDE_TRANSITION_CHOICES[0])
    slider_revolution = models.ForeignKey(SliderRevolution, on_delete=models.CASCADE)
